...
RayHitResult Scene::IntersectByRay(Ray& ray)
{
	...
	//We are only interested in the one with the smallest positive t
	//Check result.t for the parametric result of the interesection
	RayHitResult outResult = m_sceneObjects[0]->IntersectByRay(ray);	// output result is set to the intersection between primitive and ray
	for (int i = 0; i < m_sceneObjects.size() - 1; i++)	// Go through all objects in the scene	
	{
		RayHitResult currentResult = m_sceneObjects[i+1]->IntersectByRay(ray);	// Hit result obtained from another primitive
		if (currentResult.t >= 0.0)	// Reject if currentResult.t < 0.0
		{
			if (currentResult.t < outResult.t)	// Replace outResult with currentResult if currentResult.t < outResult.t
			{
